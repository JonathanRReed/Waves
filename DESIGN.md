...
The header carries the app's signature piece: **live superposition** (`MixedWaveformView`, fed by `AppStore.waveComponents` + `mixedAudioLevel`). Every playing app renders as a thin cyan-family component wave — its frequencies, drift, and color temperature derived deterministically from its logical ID, so the same app always waves the same way — and the bright center wave is their literal point-wise **sum**, soft-limited with a tanh knee. Constructive and destructive interference between apps is computed, not staged: the name of the app, drawn. It is purely decorative and always rendered on a **solid (non-glass) content surface**, never over Liquid Glass, per Apple's guidance for real-time graphics. Levels are eased (fast attack, slow release) between the sparse samples; phases only ever accumulate, so no state change can cause a visual snap. When silent it settles into a slow-breathing resting hairline (10 Hz ambient cadence, near-zero cost); under Reduce Motion it **holds a still pose**, changing only amplitude. Cyan/teal only — it is a signal element, never decoration for its own sake.
